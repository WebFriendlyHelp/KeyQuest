...
- KeyQuest no longer refuses to open when a different copy is running somewhere it cannot reach. Version 1.23.0 checked whether another copy was open, but it treated "I could not check" as "yes, one is open". If another Windows account on the same computer was running KeyQuest, or the check simply failed, you were told to switch to a copy you had no way of getting to.
- An installed copy and a portable copy no longer block each other. They keep their progress in separate places and were never really in conflict, so opening one while the other was running now works.
- If something on your computer is holding your progress file open, KeyQuest will not write over it. Antivirus, backup and file syncing software can lock a file for a few seconds. Before, KeyQuest started with empty progress and then saved that emptiness over a file that was perfectly fine. It now keeps your file, saves your session next to it, and tells you saving is not working properly.
- Your streak milestones are announced when you reach them again. Reaching one during a session was being recorded but not spoken.
- If an update has to put your previous version back, your sentence files are now genuinely restored. The backup step was aimed at the wrong place, so it saved nothing and restored nothing.
- Going back to an older version of KeyQuest no longer loses settings that only a newer version understood. This worked for one run and then dropped them on the second.
- Lessons unlocked in a newer version are no longer lost by opening an older one.
...
- When an update includes new or corrected sentence files, KeyQuest now adds them to your Sentences folder for you. This works the same way whether you use the installed version or the portable version.
- Anything you have edited is kept exactly as you left it. If an update includes a newer version of a file you have changed, KeyQuest keeps yours and tells you how many of your files it left alone.
- If you delete a sentence file, it stays deleted. It will not come back with the next update, and the topic no longer appears in your practice or speed test menus.
- Sentence files you created yourself are never touched.
- There is a new Restore Default Sentences item on the main menu. It puts every sentence file back the way KeyQuest ships it, including any you deleted. It asks first, and tells you plainly that your changes to those files will be replaced.
- If your whole Sentences folder goes missing, KeyQuest puts the original files back and tells you it has done so, instead of leaving you with an empty topic list.
- Until now the portable version never received new sentence content at all, so those files only ever changed if you edited them.
- Fixed a problem where an update could start replacing files before KeyQuest had finished closing. On some computers this made updates fail or have to be undone. This is the underlying cause of the file-in-use problem improved in 1.20.0.
- If an update cannot finish and KeyQuest puts your previous version back, it now restores it exactly, instead of possibly leaving a mixture of old and new files.
- If an installer update fails partway, your saved progress and your sentence files are now put back before KeyQuest restarts.
- KeyQuest now stops rather than installing an update it could not confirm was genuine.
- A failed check for updates no longer interrupts you. If KeyQuest cannot reach the internet during its regular background check, it now stays quiet instead of returning you to the Main Menu and opening a message in the middle of a lesson or game.
