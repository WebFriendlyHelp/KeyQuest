...
**A lesson prompt could name every key except the one that broke it.** Report: "the program may say 'press A, A', but when A is pressed a second time, we may be informed to press the Space bar." Confirmed in lesson 0, and in lesson 1 where S is introduced.
- Cause is in `_is_known_natural_word` (`modules/speech_format.py`). Authored lesson content is meant to be spoken as words rather than spelled out, and a multi-word phrase qualified when every space-separated token was a known word. But lesson 0's only authored word is "a", and its generated targets are built from the keys taught so far, which at that point are space and A. So the generated target "a a" matched the natural-word rule, was read back as plain text, and the space between the two letters was announced as nothing at all. Lesson 1 adds "as" and reproduces it with "as a", "a as" and so on. The learner pressed A twice, got the error beep, and only then heard that space was expected.
- A naturally spoken prompt now names the space between its words: "a, space, a", "as, space, a", "read, space, here". Single words are untouched, since there is nothing to join.
- A leading, trailing or doubled space no longer counts as natural at all. That case is not decorative: the prompt is re-spoken from the remaining suffix after every correct keystroke, so typing the "a" of "a a" leaves " a", whose only unspoken character was the space the learner needed next. Those spell out instead.
- One fix covers all four announcement paths, because `lesson_prompt`, the free-practice intro, the lesson-practice intro and `provide_key_guidance` all reach the same formatter.
- `test_lesson_prompt_names_the_space_bar_in_generated_targets` asserts the invariant rather than fixed strings: it drives the real batch generator for every early lesson, then checks each target at every partially typed position, so the count of spoken "space" always equals the count of spaces remaining. Reverting the fix fails it 523 times across stages 0 through 5, which is also how we learned the bug reached further than the two lessons reported.

**The app went deaf to keypresses whenever no screen reader was running.** Report: "very sluggish when moving through the menus. Pressing Up Arrow will move and read an option, but pressing Down Arrow immediately afterwards may not do anything for at least 20 seconds. It's almost like the program wants to function when it feels like it."
- Cause is `Speech._detect_narrator_process`, which spawns `tasklist`. `refresh_backend` calls it, `_refresh_auto_speech_backend` calls that once per second from the main loop, and the Narrator check is only reached when Tolk found no screen reader. So the spawn happened every second, on exactly the machines that reported the problem, and never on a machine with NVDA or JAWS running.
- **KeyQuest ships windowed, so it owns no console, and a console program launched from a parent with no console gets a console window of its own.** It appears, takes the foreground, and closes again. SDL only delivers keyboard input to the focused window, so keys pressed in that gap are not slow, they are gone.
- Measured rather than assumed: four bare `tasklist` spawns from a pythonw parent produced four different foreground windows plus an interval with no foreground window at all. The same four spawns with `CREATE_NO_WINDOW` and `SW_HIDE` never moved the foreground once.
- The spawn is now hidden, and it no longer runs on the main loop at all. `narrator_process_running()` answers from a cache that a worker thread refreshes at most every 5 seconds. Narrator starting mid-session is noticed within 5 seconds instead of within 1, which is the whole cost.
- **`update_manager._run_powershell` and `update_controller.bat_launcher_creationflags` already did this**, since 1.21.1. `speech_manager` was the one module that spawned a process without the guard. `test_every_subprocess_call_in_the_module_hides_its_window` now asserts the invariant across the module so a probe added later cannot skip it.
- `pywin32` is now named in `requirements.txt`. The native SAPI path is the fast one (measured at 13 ms per call, fully asynchronous) and it needs pywin32, which until now arrived only as a transitive dependency of pyttsx3. Losing it silently drops every no-screen-reader user onto the pyttsx3 fallback.

**A third defect, same family, found while tracing the first.** `build_remaining_text_feedback` splits the untyped remainder on the first space to decide what to spell. In sentence practice and the speed test the remainder is the whole untyped suffix, so pausing between words leaves a leading space, the split produced an empty first word, and spelling that returned the literal word "nothing". Anyone who stopped at a word boundary and asked what was left heard "Type: nothing. Then: world". It named neither the space nor anything else. The space bar is now named there, as it is everywhere else. Affects sentence practice, the speed test, and hangman's sentence mode, all of which share the helper.
- Not changed: `"a a"` still reads as "Type: a. Then: a" rather than naming the space between them. In sentence mode the "Then:" half is read as ordinary prose, and naming every space in a sentence would be unbearable. Only the case where the space is the *immediate next key* is affected.

**Deleted the dead `LessonManager` class**, 238 lines in `modules/lesson_manager.py`. It was referenced nowhere but its own definition line, and `lesson_mode.py` had reimplemented every one of its five methods. The reason it mattered rather than merely being clutter: `get_prompt_parts` carried a duplicate of the space-bar bug above, so anyone tracing this changelog entry later could have fixed that copy, run the app, and seen nothing change. Removing it also removed the module's only use of `speech_format`, which is how ruff confirmed nothing else depended on it. `lesson_manager.py` is now lesson content and progression data; batch generation lives in `lesson_mode.py`, and only there.
- **Verified rather than asserted, because losing lesson content was the stated risk.** Every lesson structure was dumped to JSON before and after the deletion with the RNG pinned: all 33 lessons, their key sets, names, key-location descriptions, authored words and phrases, special-key drills, progression constants, star thresholds, 548 generated batch entries and the 1,723 prompt strings those produce at every partially typed position. The two dumps are byte-identical, same MD5. The diff of the source file itself is 238 deletions and zero additions.
- New `TestLessonContentIsIntact` pins the lesson count and the exact keys each lesson introduces, so losing a lesson or a key becomes a test failure here instead of something a learner discovers. Adding a lesson is a deliberate one-line update to that list. Checked by deleting a lesson: the count assertion and 17 subtests fail.

**Decided, not fixed: the pyttsx3 fallback is not thread-correct.** The engine is built on the main thread but `say()` and `runAndWait()` are driven from the worker, while the main thread calls `stop()` on it concurrently. pyttsx3's Windows driver is a COM object in a single-threaded apartment and `runAndWait` pumps that apartment waiting for an end-of-stream event, so the event lands on the main thread's queue and the worker's wait ends only when the pygame loop happens to pump it. That produces the same intermittent symptom as the bug above, from a different cause. It does not affect the shipped build, where pywin32 is bundled and the native SAPI path wins, so the pyttsx3 worker never starts. Fixing it properly means moving every engine operation onto the worker, including `get_available_voices` and `apply_tts_settings`, and the obvious small fix does not work: removing the cross-thread `stop()` would leave menu navigation unable to interrupt speech at all. This is adjacent to, and larger than, the pyttsx3 item already carried in HANDOFF. **Owner decision 2026-08-14: recorded, not fixed.** It cannot reach the shipped build, and landing a speech-engine threading refactor in the same release as two confirmed user-facing fixes is the exact pattern this project has already paid for once, where a protection added to a release became the biggest risk in it.

Verification: unit suite 462 passed + 10,470 subtests. Every new test was run against deliberately reverted or doctored source and fails there: 523 subtest failures for the prompt fix, all 5 speech-manager tests for the focus fix.
...
- **The transition freeze.** An install with no baseline had to keep everything conservatively, and the baseline then advanced to the new release. Every untouched file whose content had changed across that gap differed from the baseline forever: never corrected again, and announced as "your own file" to a user who had never touched it.
- **The one-time-failure freeze.** A file locked during one update was skipped while the baseline advanced past it, freezing it as "customized" for good.

Both are properties of comparing against *one particular previous release*. They dissolve when "unmodified" means **matches any version we have ever shipped**.

- New `sentence_history.json` at the app root records every SHA-256 the project has ever shipped per file name, generated by `tools/dev/build_sentence_hashes.py` and copied into `dist` by the spec (which now refuses to build without it). Hashes normalise line endings, so an editor rewriting CRLF does not cost a user their corrections.
- `modules/sentence_merge.py` is now: not present in the user's folder → add; content matches a hash we ever shipped → not theirs, update it; matches nothing we shipped → theirs, keep; no history available → replace nothing. A missing or corrupt history file makes the merge cautious, never destructive.
- **`_sentences_shipped` is gone**, and any stale copy is deleted on sight. This removes an entire class of review findings with it: the manufactured baseline, the baseline advancing past a skipped file, and (found by Codex) the installer staging a *polluted* baseline, because Inno overlays rather than replaces, so a user-created file could be recorded as though KeyQuest shipped it and later authorise overwriting it. None of those can happen without a baseline.
- **`manifest.json` now merges too** (`.json` added to the mergeable set), so topic names, explanations and `speed_test_file` corrections reach existing installs. History decides it like any other file, so a user who edited their manifest keeps it.
- **A staged set is only consumed once every file was dealt with.** A locked file now leaves the incoming folder in place so the next startup retries, instead of the correction being lost permanently.
- New `test_the_shipped_history_file_matches_the_real_sentences_folder` guards the release step: if sentence content changes and `build_sentence_hashes.py` is not re-run, every user's copy looks edited and corrections silently stop. The test names the command to run.

What now sits in an app directory: `Sentences/` (the user's, the only one they see), `_sentences_incoming/` (transient, between an update applying and the next startup), and `sentence_history.json` (one file).
...
- **`pending_update.json` was being deleted by the portable update itself.** `update_controller.py` writes the marker into the app dir just before exit, but the portable `robocopy /MIR` (`update_manager.py:760`, and the fallback template at ~944) excluded only `progress.json`, `KeyQuest.exe`, and `keyquest_error.log` via `/XF`. The marker is in neither exclusion list and is absent from the extracted payload, so `/MIR` purged it every time. Effect: post-restart verification could never confirm success, **and** on any post-mirror failure (robocopy >= 8, structure check, exe-copy failure into rollback) the spoken "a recent update did not apply successfully" warning never fired either — the one signal a blind user gets that something went wrong. Only pre-mirror failures (tar extraction) left it intact. Fixed by adding `pending_update.json` to `/XF` in both portable templates. The installer path was never affected (Inno does not delete unknown files). Regression guard added to `test_create_portable_update_launcher_contains_tar_and_robocopy`.
- **A failed background update check dragged the user out of whatever they were doing.** `_handle_update_check_result` unconditionally set `state.mode = "MENU"`, spoke, and ran `_offer_update_failure_recovery` (which clobbers the clipboard with the error log and opens a modal) on *any* check error. The periodic check fires every 4 hours regardless of mode, so a user playing offline was pulled to the menu with a dialog every 4 hours. The `mode = "MENU"` line was meaningless for check errors specifically: `start_update_check` returns early if mode is already `UPDATING`, so mode is never `UPDATING` during a check and there was nothing to restore. Background failures are now recorded via `_record_update_event` and otherwise silent; manual checks keep the full speech + recovery flow unchanged. This was a regression introduced when the logic moved out of `keyquest_app.py` — the original only ran recovery when `manual` was true. New `tests/test_update_check_error_handling.py` covers background-silent, manual-reports, and the missing-`manual`-key case.
- Note (forward-only, per the mental model in `docs/dev/UPDATE_PROCESS.md`): both fixes live in the *new* version, so they only affect updates performed **from** this version onward. Copies at 1.22.0 and earlier keep the old behavior for their next update.
...
- `modules/update_manager.py`: the portable update launcher and portable fallback launcher now take an optional `backup_zip_path`. After the destructive `robocopy /MIR`, any failure (robocopy `>= 8`, missing `modules/version.py`, or `KeyQuest.exe` replacement failure) routes to a new `:rollback` routine that restores the snapshot with a verified retry loop (re-extract until `modules\version.py` exists, up to 10 attempts) before restarting. `Backups` was added to the `/MIR /XD` exclude lists in both launchers so the mirror cannot delete the very snapshot rollback depends on.
- `modules/update_manager.py`: **all** generated `tar` calls (download extraction *and* rollback restore) now resolve tar explicitly to the Windows-bundled bsdtar via a `kqTar` variable (`%SystemRoot%\Sysnative\tar.exe` then `%SystemRoot%\System32\tar.exe`, falling back to bare `tar`). This fixes a latent bug: a GNU/MSYS `tar` ahead on `PATH` (e.g. Git for Windows) reads an archive path like `C:\...zip` as a `host:path` remote spec and fails with `tar: Cannot connect to C: resolve failed`. The integration test had masked this because it extracts via the `KEYQUEST_UPDATER_TEST_PYTHON` override, never exercising real tar.
- `modules/update_controller.py`: added `_create_rollback_backup(version)`, which speaks "Creating a safety backup before updating.", logs the result, and is invoked on both the portable main launcher path (`_launch_downloaded_update`) and the portable direct-fallback path (`_fallback_apply`). The installer path is unchanged — Inno Setup applies transactionally.
- `tests/test_update_manager.py`: added coverage for backup creation (content + transient-dir exclusion), pruning to `max_kept`, the missing-directory `None` case, rollback presence in the generated launcher, the cleared `__BACKUP_ZIP__` placeholder when no backup is supplied, and the `%kqTar%` System32 resolution.
- Verified end-to-end on Windows by generating real launchers and running them: a deliberately broken build triggers rollback and restores `modules/version.py` (old version preserved, user `Sentences`/`progress.json` intact), and a valid build applies through the real `tar` extraction path (updates to the new version, `Backups` survives the `/MIR`).
- `tests/run_local_updater_integration.py`: added `_sweep_mei_temp_dirs(since)`, called in the `main()` `finally`, to remove the `%TEMP%\_MEI<pid>` folders left by the `--onefile` fixture exes when the harness force-kills them mid-update. Scoped by timestamp (only dirs modified at/after the run start), so a concurrently running onefile app's live temp dir is never touched; locked dirs are retried a few times then skipped. Real KeyQuest is a onedir build and never creates `_MEI` dirs — this is test-harness hygiene only.
...
- `tools/release.ps1`: Automated the remaining ship-time updater checks. After tagging, the release script now waits for GitHub to publish the release and verifies that all four expected assets are attached (`KeyQuestSetup.exe`, `KeyQuestSetup.exe.sha256`, `KeyQuest-win64.zip`, `KeyQuest-win64.zip.sha256`).
...
- `.github/workflows/release.yml`, `.github/workflows/update-smoke-test.yml`, `tools/release.ps1`: Fixed the post-release smoke-test automation design after confirming a GitHub Actions token restriction. Because the release is published by `release.yml` using `GITHUB_TOKEN`, GitHub did not reliably trigger a second workflow on `release.published`. The smoke test now runs as a second job inside `release.yml`, `update-smoke-test.yml` is back to manual use, and the local release script no longer waits for a separate workflow that may never start.
