...
- **OPEN (2026-08-08): user-data items deliberately not fixed yet.** From the two user-data reviews; the critical ones are fixed (see CHANGELOG). Left alone on purpose, each needing a decision rather than a patch:
  1. **No single-instance guard.** Two copies running at once each hold a snapshot and save the whole file, so the one closed last silently reverts everything earned in the other. A double-clicked shortcut is enough. Needs a mutex or lock file, and a decision about what to tell the second instance.
  2. **`schema_version` is written but never read** (`state_manager.py`). An older build loading a newer file silently drops the unknown fields on its next whole-file save. Harmless between 1.22.0 and 1.23.0, which have identical persisted fields, but it is a live rollback hazard the moment they diverge.
  3. **The streak and daily challenge only roll over at launch.** Someone who leaves KeyQuest open and practises daily never updates `last_practice_date`, so relaunching later resets an earned streak. Fix is to run the check on each session-recording event, not just startup.
  4. **`unlocked_lessons` is not validated on load.** A hand-edited or partially recovered file containing strings loads fine, then makes `sorted()` raise on every future save: silent, permanent save failure while the user keeps playing. The new spoken save warning at least makes it audible now.
