...
- **OPEN (2026-08-08): the sentence merge is NOT finished. Do not ship it as-is.** The core decision in `modules/sentence_merge.py` is right and both reviewers said so, and the primary portable and installer paths work. But Fable and Codex independently reviewed it and both concluded it does not hold up yet. Fixed in this session: the installer fallback destroying every user edit (it ran Inno with no backup at all), the portable fallback never staging so new content never arrived, the manufactured baseline authorising an overwrite one run later, a false "Sentence content updated" when nothing was updated, broken singular grammar, `_sentences_shipped` not being gitignored (it would have been committed into a release), and a 3-second speech protection window that only guaranteed the summary was cut off by the 1800 ms startup menu announcement. **Still open, both reviewers agreeing unless noted:**
  1. **`manifest.json` never merges.** Only `.txt` is considered, so topic names, explanations and `speed_test_file` corrections never reach an existing install. Fresh installs only.
  2. **The transition freeze.** On the first update after this feature ships there is no real baseline, so everything is conservatively kept, and the baseline then advances to the NEW set. Every untouched file whose content changed across that transition differs from the baseline forever, is never corrected again, and is announced as "your own file" to a user who never edited anything. **Fable's suggested fix (defer consuming incoming) does not work** — a manufactured baseline equals the live folder either way, so it can never identify edits. The real fix is shipping a manifest of historical per-file hashes, so "unmodified" means "matches any hash we ever shipped". That is a design decision, not a patch.
  3. **A one-time failure becomes a permanent freeze.** A locked or partially staged file is skipped, but the baseline advances past it regardless, so that file is treated as customized forever after.
  4. **The installer stages a polluted baseline** (Codex only, confirmed): Inno overlays rather than replaces, so a user-created file gets staged as though KeyQuest shipped it, which can later authorise overwriting it.
  5. **The primary installer preservation sequence fails open**: unchecked `robocopy` results, the backup deleted unconditionally, and no restore on the installer-failure paths.
  6. Merged content is not served until restart (sentences are cached before the merge runs), deletions are not honoured (a deliberately deleted file is re-added and announced as new), and the summary can still be clipped by the menu announcement.
  - **Recommendation: one focused session on the above before this ships**, starting with the historical-hash manifest, since items 2 and 3 both dissolve once "unmodified" is decidable. The build would need to emit the hash list per release.
