...
- **OPEN (2026-08-07): updater review findings, two fixed, the rest triaged and unfixed.** Two independent reviews (Fable and Codex) over `update_manager.py` / `update_controller.py` / the updater tests. Heavy overlap between them, which is good corroboration; each also found things the other missed. **Every item below was verified against the source before being written down.** Fixed already (see the 2026-08-07 CHANGELOG entry): the `pending_update.json` `/MIR` purge, and the background-check-error interruption. **Still open, in the order I would fix them:**
