...
- **v1.24.0 real-world confirmed: the owner's installed copy applied 1.23.0 → 1.24.0 through the in-app updater** (2026-08-08). Three confirmed live in-app updates now: 1.21.1 → 1.21.2, 1.21.2 → 1.23.0, 1.23.0 → 1.24.0.
  - **This one proved something the others could not.** 1.23.0 was the first version to ship the single-instance guard, so this is the first update where the *outgoing* copy held a lock while the updater relaunched the new one. The retry window in `single_instance.py` (a second instance waits several seconds before concluding it is genuinely a duplicate) existed precisely for that overlap and had never been exercised in the field. If it had been wrong, the user would have been left with nothing running immediately after an update, which is the stranding this project spent a year removing. It held.
