...
- **DONE (2026-08-08): app-wide spoken-output sweep. Result was clean; do not redo it.** All 239 `speech.say` call sites across `modules/`, `games/` and `ui/` were checked mechanically, after two delegated agents went idle without reporting. Findings:
  ...
  - **The one real finding, left for a decision:** `sentences_manager.load_practice_sentences` (around line 317) silently substitutes built-in fallback sentences when a topic file cannot be read. The user hears their chosen topic announced and then practises entirely different content, with nothing said. Now less likely than it was (deleted topics no longer appear in menus), but an unreadable or corrupt file still hits it. Fixing it means the loader reporting that it fell back and the caller announcing it, across 7 call sites; the judgement call is whether that announcement is worth the noise on every load.
