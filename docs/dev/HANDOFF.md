...
- **Last updated**: 2026-08-15. **Current release: v1.27.2**, shipped, verified, and **confirmed live**: the owner's installed copy applied 1.27.1 → 1.27.2 through the in-app updater the same evening, reporting version 1.27.2 with release date 2026-08-15. That is the seventh confirmed live in-app update, and it doubles as the installer smoke test, since the updater runs the real installer silently over a live install with real user data in place. Contents: the Report a Problem crash fix and rework, the About screen rewiring, and the diagnostics file name. **Windows Sandbox does NOT work on this machine** for automated testing; see the corrected note in the Inno Setup section. The 2026-08-14 and 2026-08-15 entries below cover that work: two tester-reported bugs, the SAPI leading-`<` silence, the no-screen-reader backend bug, the speech transcript, Report a Problem, and two new test harnesses. **Open items are in the section immediately below this snapshot.**
...
- **SHIPPED, verified, and real-world confirmed: v1.25.0** (2026-08-08). Release workflow green including `post-release-smoke-test`; all four assets present; both checksums verified against a fresh download; the shipped exe launches, reports 1.25.0, and its update check finds nothing newer. Contents: the robocopy result checks in both installer launchers, and the spoken notice when a sentence topic could not be loaded. The owner's installed copy applied **1.24.0 → 1.25.0 through the in-app updater**.
  - **This is the update that actually exercised the single-instance retry window.** Both 1.24.0 and 1.25.0 use the same path-hashed lock name, so for the first time the outgoing copy genuinely held the name the incoming copy needed while the updater relaunched it. Every earlier update either had no guard or, as with 1.23.0 → 1.24.0, had the two sides asking for different names. Had the retry window been wrong, the user would have been left with nothing running immediately after an update. It held.
  - Four confirmed live in-app updates now: 1.21.1 → 1.21.2, 1.21.2 → 1.23.0, 1.23.0 → 1.24.0, 1.24.0 → 1.25.0.
- **v1.24.0 real-world confirmed: the owner's installed copy applied 1.23.0 → 1.24.0 through the in-app updater** (2026-08-08). Three confirmed live in-app updates now: 1.21.1 → 1.21.2, 1.21.2 → 1.23.0, 1.23.0 → 1.24.0.
  - **Correction, 2026-08-08.** This entry originally claimed that update proved the retry window under a real overlap. It did not, and the reasoning was wrong: 1.23.0 holds `Global\KeyQuest.SingleInstance.v1` and 1.24.0 takes the path-hashed `...v2.<hash>`, so the outgoing and incoming copies asked for **different names** and could not contend at all. The retry window was not exercised by that update. Corrected rather than deleted, because "the guard was present on both sides" is not the same as "they wanted the same lock", and that is an easy assumption to repeat.
- **SHIPPED and verified: v1.24.0** (2026-08-08). Release workflow green including `post-release-smoke-test`; all four assets present; both checksums verified against a fresh download; the shipped portable exe launches, reports 1.24.0, and its update check correctly finds nothing newer. **The guard was re-checked in the built exe**: two copies of the same install still produce a first window titled "Key Quest" and a second titled "KeyQuest Is Already Open", so failing open did not turn the guard off. The installer was checksum-verified but deliberately not run: an Inno `/CURRENTUSER` install repoints the uninstall registry entry, so a throwaway install would have aimed the owner's own uninstaller at a temp folder. The portable zip is the same build output.
- **v1.24.0 fixes what 1.23.0's own new code broke.** The single-instance guard added in 1.23.0 failed **closed**: any `CreateMutexW` failure, not just "already running", refused to start the app. It also used one global lock name, so an installed and a portable copy blocked each other. Both fixed, plus the locked-progress-file overwrite that the 1.23.0 quarantine could not actually prevent. See the 1.24.0 CHANGELOG entry.
...
0. **CRASH in shipped v1.27.1 on Report a Problem: cause identified, fix verified, watch it anyway.** The owner's installed copy died with an access violation on 2026-08-15 at 13:26:29, seconds after pressing Enter on About > Report a Problem.
   ...
   **It is two successful runs, not a hundred.** Do not quietly promote this to "fixed" in the release notes without more exercise, and if any user reports KeyQuest vanishing, look here first. The evidence below is kept because it is what a recurrence would have to be checked against.
   ...
   - **Reproduced four more times at 14:13 the same day**, driving the shipped build through the About menu. Four crash dumps sixteen seconds apart, matching the harness launch cadence, every one the same fault at the same offset. So this is not rare: the shipped build appears to fail this **every time the feature is exercised**, and the earlier "it worked at 11:46" is the outlier that still needs explaining.
   - Not reproduced from source, frozen or not: 30 attempts on a dummy display, 15 more with a real window and Tolk and NVDA loaded, driving the real functions with a real report. **Only the frozen build crashes**, which points at something about the bundle rather than the Python.

   **Automating this is harder than it looks, and cost most of an afternoon.** Two harnesses failed for reasons worth writing down. `SendKeys` needs the foreground and Windows will not give it to a process launched from a background shell, so six of eight runs sent their keys into the terminal instead. Posting `WM_KEYDOWN` directly to the window needs no foreground but SDL only delivers keys to a window it believes has keyboard focus, and a KeyQuest launched from a background shell never gets it, so every posted key was dropped. The runs that did work were the ones where the app happened to hold the foreground. Anything automated here has to solve focus first, or drive the app from a process that already owns the foreground.
...
- **Lesson prompts skipped the space bar.** `_is_known_natural_word` classified a generated target as a natural phrase when every space-separated token happened to be a known word, and lesson 0's known words are just "a" while its keys are space and A. So "a a" was read back as plain text with the space silent. Fixed in `modules/speech_format.py`; one change covers all four announcement paths. The regression test drives the real generator and failed 523 times against the old code, across stages 0 through 5.
- **The app lost keystrokes whenever no screen reader was running.** `Speech._detect_narrator_process` spawned `tasklist` once per second from the main loop, and only when Tolk found no screen reader. KeyQuest ships windowed, so that console child got its own console window and took the foreground. Measured: four bare spawns from a pythonw parent moved the foreground four times plus an interval with no foreground window at all; the same spawns with `CREATE_NO_WINDOW` and `SW_HIDE` never moved it once. Now hidden, cached for 5 seconds, and refreshed on a worker.
  - **`update_manager` and `update_controller` had this guard since 1.21.1 and `speech_manager` did not.** Same shape as every other bug in this file: a protection present on one path and quietly absent on its sibling. There is now an invariant test over the whole module rather than over the one call.
- **`build_remaining_text_feedback` said "Type: nothing"** whenever the untyped remainder began at a word boundary, which is every pause between words in sentence practice and the speed test. Fixed to name the space.
- **Deleted the dead `LessonManager` class** (238 lines, `modules/lesson_manager.py`). Referenced nowhere but its own definition; `lesson_mode.py` had reimplemented all five methods. It carried a duplicate of the space-bar bug, so a future fix applied there would have done nothing. Lesson content was proved unchanged rather than assumed: every lesson structure, generated batch and prompt string dumped to JSON before and after with the RNG pinned, byte-identical both times, same MD5. `TestLessonContentIsIntact` now pins the lesson count and each lesson's key set so losing one fails a test.
- **DECIDED, recorded not fixed: the pyttsx3 fallback is not thread-correct.** The engine is built on the main thread, driven from the worker, and `stop()` is called on it from the main thread concurrently. Its Windows driver is an STA COM object whose `runAndWait` pumps that apartment for an end-of-stream event, so the event lands on the wrong thread's queue and the worker only proceeds when the pygame loop happens to pump it. Same intermittent symptom as the focus bug, different cause. **It does not affect the shipped build**, where pywin32 is bundled and the native SAPI path wins, so the pyttsx3 worker never starts. A correct fix moves every engine operation onto the worker, including `get_available_voices` and `apply_tts_settings`; the tempting small fix does not work, because removing the cross-thread `stop()` leaves menu navigation unable to interrupt speech. Larger than, and adjacent to, the pyttsx3 item already carried in the open-items list. Owner decision 2026-08-14: leave it recorded. Reopen it only if a real install is ever confirmed to be running on the pyttsx3 path, or alongside other speech work already underway.
- `pywin32` is now explicit in `requirements.txt`. It was arriving only as a transitive dependency of pyttsx3, and losing it would silently drop every no-screen-reader user onto that broken fallback.

**SHIPPED and verified: v1.26.0** (2026-08-14). Release workflow green including `post-release-smoke-test`; all four assets present; both checksums verified against a fresh download; `releases/latest` reports v1.26.0, so the in-app updater can see it. The portable build was extracted and checked to actually contain the fixes rather than assumed to: `hidden_process_kwargs` present in `speech_manager.py`, `SPACE_TOKEN_JOINER` present in `speech_format.py`, the dead `LessonManager` gone, 33 lessons intact, `__version__` reading 1.26.0. The shipped exe launches and holds the window title "Key Quest", so the single-instance guard granted it the primary lock. The installer was checksum-verified but deliberately not run, for the same reason as 1.24.0 and 1.25.0: an Inno `/CURRENTUSER` install repoints the owner's uninstall registry entry at a temp folder. The portable zip is the same build output.
...
- **Lesson worth keeping (2026-08-08): the guard added to protect user data was itself the biggest risk in the release.** It shipped failing closed, which for a blind user means the app silently does not open and there is no visible reason why. Two reviews caught it independently. The general rule this project keeps relearning: **a new protection needs its failure direction chosen deliberately.** When the check cannot be made, decide whether "cannot check" means allow or deny, and prefer allow whenever denying can strand someone.
- **RESOLVED (2026-08-08): all four open user-data items are fixed.** Single-instance guard (named mutex, with a retry window so an update relaunch is never blocked, and a native message box so the user is actually told), `schema_version` now preserves a newer build's fields instead of stripping them, the streak counts every recorded session rather than only launches, and `unlocked_lessons` is validated on load so a bad value can no longer break every future save. See the CHANGELOG entry.
- **DONE (2026-08-08): app-wide spoken-output sweep. Result was clean; do not redo it.** All 239 `speech.say` call sites across `modules/`, `games/` and `ui/` were checked mechanically, after two delegated agents went idle without reporting. Findings:
  - **Zero** non-ASCII characters, emoji, box-drawing or symbols that read badly aloud, in any spoken string. The project's rule is genuinely being followed.
  - **Zero** cases of a protected announcement swallowing a following non-interrupting one. (There was exactly one, introduced and fixed earlier the same day.)
  - Two singular/plural candidates, **both false positives**: the free-practice key count is never below 2, and hangman's `MIN_WORD_LENGTH` is 5, so neither `{n} keys` nor `{n} letters` can ever say "1".
  - 25 failure paths that log or print without speaking. Nearly all are correct by nature: `speech_manager` cannot announce that speech is broken, `dialog_manager` falls back to print when wx is unavailable, clipboard-copy failures still show the results in the dialog, and `state_manager`'s save/load failures are now announced by their caller.
  - ~~**The one real finding, left for a decision:** `load_practice_sentences` silently substitutes built-in sentences when a topic file cannot be read.~~ **Done 2026-08-08.** The earlier framing of the cost was wrong and is worth remembering: it was recorded as "an announcement on every load", but it can only speak when a substitution actually happened, which should be almost never. There was no ongoing noise to weigh. New `load_practice_sentences_with_status` returns `(sentences, substituted_topic)`; `load_practice_sentences` still returns a plain list for callers that cannot say anything. An empty-but-present topic file now falls back too, instead of handing back an empty practice session.
    - **Where the notice is spoken is the load-bearing part.** It is folded into the practice and speed test intros, not spoken before them. Those intros are priority speech with a 3-second protect window and they interrupt, so a separate notice queued ahead would have been purged: the exact trap documented above. The cache-load call sites in `keyquest_app.py` deliberately say nothing, because the user has not started typing yet and the intro will tell them when they do.
- **OPEN (2026-08-08): sentence merge, remaining items. Smaller than it was.** Two independent reviews (Fable, Codex) plus a follow-up verification pass. **Closed since:** the installer fallback destroying every user edit; the portable fallback never staging so new content never arrived; `manifest.json` never merging; the transition freeze and the one-time-failure freeze (both dissolved by deciding from `sentence_history.json` instead of a baseline copy); the manufactured, polluted and advancing-baseline findings (all moot, there is no baseline); a false "Sentence content updated" when nothing was; broken singular grammar; a staged set being consumed even when a file was skipped; the installer-failure paths not restoring the user's sentence backup **and** `cleanup_stale_update_files` then deleting that backup on the very next startup, which was the default sequence and destroyed their only copy.
  ...
  - **Still open, needing a decision rather than a patch:**
    ...
    3. ~~The primary installer bat's `robocopy` results are unchecked.~~ **Done 2026-08-08.** robocopy reports 0-7 for success and 8 or above for failure, and nothing read it. Backup, staging and restore are now all checked in **both** installer templates, not only the primary one: the fallback had the identical destructive restore, and fixing one path while leaving its sibling is how every bug in this file has survived. The restore deletes the installed `Sentences` folder before copying the user's back, so it now refuses to start unless the backup completed, and the backup is kept rather than deleted whenever the restore cannot be confirmed. `:restoreuserdata` also stopped logging success unconditionally. Covered by `TestNoRobocopyResultIsIgnored`, which asserts the invariant rather than the current lines, so a robocopy added later is covered too; both assertions were checked against doctored templates to prove they are not blind.
    4. **`pyttsx3` fallback can drop the summary**: that backend overwrites pending text rather than queueing (`speech_manager.py:277-279`). Only affects installs with no screen reader and no SAPI. **Owner decision 2026-08-08: left as is**, recorded rather than fixed.
- **DONE: the 1.23.0 What's New entry is folded in** and the draft file removed.
