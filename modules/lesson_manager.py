"""Lesson content and progression data for KeyQuest.

Holds the lesson constants and data: which keys each lesson introduces, the
lesson names, the key-location descriptions, the authored words and phrases,
and the special-key command drills. Batch generation and progression live in
``lesson_mode.py``, which is the only implementation of them.
"""
